...
import data_helper
import pickle
from signalmatrix import signalMatrix
from attention import attention
from general_utils import Progbar
from sklearn.model_selection import StratifiedKFold
from sst_config import Config

dataset_name = 'emotionCause'
...
class Classifer(object):
    ...
    def __init__(self, config, session):
        ...
        # add
        # self.multi = tf.placeholder(tf.float32, [None, config.num_label], name="multi")
        # self.rank = tf.placeholder(tf.float32, [None, config.num_label], name="rank")
        self.length = tf.placeholder(tf.float32, [None], name="length")

        self.signal = tf.placeholder(tf.float32, [None, config.num_label, config.num_label], name="signal")

        self.corr = tf.placeholder(tf.float32, [config.num_label, config.num_label], name="signal")
        ...
        # modify loss
        self.count_positive = self.length
        self.count_negative = config.num_label - self.count_positive
        # self.multitile = tf.reshape(self.multi, (-1, 1, config.num_label))
        # self.signal = tf.subtract(tf.transpose(self.multitile, [0, 2, 1]), self.multitile)

        self.logittile = tf.reshape(self.to_print, (-1, 1, config.num_label))
        self.diff_predict = tf.subtract(tf.transpose(self.logittile, [0, 2, 1]), self.logittile)
        diff = self.signal * self.diff_predict
        loss = tf.exp(-1.0 * diff + 1e-10)

        print("test",self.corr)
        corr = tf.tile(self.corr, [config.batch_size, 1])
        print(corr)
        corr = tf.reshape(corr,(config.batch_size,config.num_label,config.num_label))
        print(corr)
        loss_relation = tf.square(diff) * corr
        loss = tf.abs(self.signal) * loss+loss_relation
        #total num of label pairs
        self.loss = loss = tf.reduce_sum(loss, reduction_indices=[1, 2]) / ((self.count_positive + 1e-10) * (self.count_negative + 1e-10)+(self.count_positive+1e-10)*(self.count_positive-1)+self.count_positive+self.count_negative)
        loss = loss / 2.0
...
if __name__ == "__main__":
    ...
    # texts = pickle.load(open('./data/sina/nopunc_sinatext.pkl', 'rb'))
    # wordindex, wordvectordict = data_helper.word2vec_embedding(texts, vector_path, config.hidden_size)
    ...
    # wordindex, wordvectordict = data_helper.outside_embedding('./data/sgns.renmin.pickle')
    ...
    length = []
    for label in labels:
        emotionrank = np.argsort(np.array(label) * -1).tolist()
        # emotionrank.reverse()
        emotionlength = len(list(filter(lambda i: isinstance(i, int) and i >= sum(label) * threshold, label)))
        emotionlabel = np.zeros((len(label),), dtype=np.int)
        for index in range(emotionlength):
            emotionlabel[emotionrank[index]] = int(1)
        multilabel.append(emotionlabel.tolist())
        rank.append(emotionrank)
        length.append(emotionlength)
    corr = np.corrcoef(labels, rowvar=0)
